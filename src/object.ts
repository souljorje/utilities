 ...
 */
export const mapKeysToValues = <K extends string, V, R = V>(
    keys: K[],
    values: V[][],
 ...
 */
export const findByPropertyValue = <T extends Record<string, any>, K extends keyof T>(
    obj: Record<string, T>,
    key: K,
    value: T[K],
): T | undefined => (
    Object.values(obj).find((item) => item[key] === value)
);
 ...
 */
export const arrayAddConditionally = <T, F = never>(
    condition: boolean,
    value: T | T[],
    fallbackValue: F | F[] = [] as unknown as F[],
): Array<T | F> => (
    condition ? ensureArray(value) : ensureArray(fallbackValue)
);
 ...
 */
export const arrayCreateConditionally = <T>(...map: Array<T | [boolean, ...T[]]>): T[] => {
 ...
 */
export const getPropByPath = <T = unknown>(
    object: any,
 ...
 */
export const mergeObjects = <T extends Record<string, any>>(source: T, updates: Partial<T>): T => {
 ...
 */
export const mapObjectValues = <T extends Record<string, any>, R>(
    obj: T,
 ...
 */
export const mapObjectEntries = <T extends Record<string, any>, R>(
    obj: T,
 ...
 */
export const toSet = <T>(values: Iterable<T> | Set<T>): Set<T> => (
    isSet(values) ? values : new Set(values)
);
 ...
 * getAllCombinations([]); // [[]]
 */
export const getAllCombinations = <T>(arraysToCombine: ReadonlyArray<ReadonlyArray<T>>): T[][] => {
